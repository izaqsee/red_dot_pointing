...
struct Mouse { uint8_t buttons; int8_t x, y; };
...
void mouseState(int8_t x = 0, int8_t y = 0) {
  if (!linked) return;
  if (!mouse.push({mouseButtons, x, y})) {
    mouse.clear(); neutralMouse = true; mouse.push({mouseButtons, 0, 0});
  }
}
void keyboardState() {
  if (!linked) return;
...
void buttonActionMouse(uint8_t code, bool pressed) {
  uint8_t mask = code == 1 ? 1 : code == 2 ? 4 : 2;
...
extern "C" void redpoint_hid_service(bool ready) {
  ...
  if (neutralMouse) {
    ...
    if (redpoint_mouse_send(r.buttons, r.x, r.y, 0, 0)) mouse.pop();
  }
  if (neutralKeyboard) {
