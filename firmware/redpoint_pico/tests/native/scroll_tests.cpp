...
void runScrollTests() {
  ...
  command("SET invertX 1"); testPacket(3,-2); expect(0,0,3,2);
  command("SET invertY 1"); testPacket(3,-2); expect(0,0,-3,2);
  command("SET middleSensitivity 0"); testPacket(127,127); expect(0,0,0,0);
  command("SET invertX 0"); command("SET invertY 0"); command("SET middleSensitivity 0.5");
